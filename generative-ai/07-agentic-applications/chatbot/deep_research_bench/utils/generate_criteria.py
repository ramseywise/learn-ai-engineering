...
def parse_llm_output_as_json(
    text: str, expected_type: type = list
...
def round_weights_and_adjust(weights, decimal_places=2):
    ...
    if abs(diff) > 1e-10:
        rounded_weights["readability"] = round(
            rounded_weights["readability"] + diff, decimal_places
        )
...
def generate_weights_multiple_times(
    item_id, prompt, language, sample_count=DEFAULT_SAMPLE_COUNT
):
    ...
    for dim in dimensions:
        ...
        if len(values) == len(
            weights_samples
        ):  # Ensure dimension appears in all samples
...
def generate_criteria_pipeline(
    input_file,
    output_file,
    process_limit,
    max_workers,
    ...
    with open(input_file, "r", encoding="utf-8") as f:
        for i, line in enumerate(f):
            if (
                ...
                and len(input_data_to_process) >= process_limit
            ):
                break
            ...
            if data.get("id") and data.get("prompt"):
                ...
                input_data_to_process.append(
                    {"id": data["id"], "prompt": data["prompt"], "language": language}
                )
    ...
    print(
        f"Starting to process {len(input_data_to_process)} items using up to {max_workers} threads..."
    )
    ...
    print(
        f"Total runtime:        {duration:.2f} seconds ({items_per_sec:.2f} items/sec)"
    )
...
if __name__ == "__main__":
    ...
    generate_criteria_pipeline(
        INPUT_FILE, OUTPUT_FILE, PROCESS_LIMIT, MAX_WORKERS, DEFAULT_SAMPLE_COUNT
    )
