...
class ArticleCleaner:
    ...
    def _clean_text(self, text, language="zh", max_retries=3):
        ...
        for retry in range(max_retries):
            try:
                result = self.clean_agent.generate(
    ...
    def clean_single(
        ...
        try:
            ...
            if cleaned_article is None:
                logger.info(
                    f"ID: {item_id} - Article may be too long, trying chunked processing"
                )
            ...
            if not self._is_valid_result(cleaned_article):
                logger.error(
                    f"ID: {item_id} - Failed to clean article after {max_retries} retries"
                )
                self._update_progress(pbar, pbar_lock)
            ...
            if output_file and file_lock and processed_ids is not None:
                self._write_result_to_file(
                    result, output_file, file_lock, processed_ids, item_id
                )
                self._update_progress(pbar, pbar_lock)
                return item_id
            else:
    ...
    def _write_result_to_file(
        self, result, output_file, file_lock, processed_ids, item_id
    ):
    ...
    def clean_articles(
        ...
        with tqdm(
            total=len(all_items),
            desc=f"Cleaning {model} articles",
            ...
            for line in f:
                if line.strip():
                    try:
                        ...
                        logger.warning(
                            f"Error parsing JSON in input file, line: {line.strip()}"
                        )
        return all_items
    ...
    def _load_processed_ids(self, output_file):
        ...
        if os.path.exists(output_file):
            ...
            logger.info(
                f"Read {len(processed_ids)} already processed records from output file"
            )
        else:
            # Create empty file
